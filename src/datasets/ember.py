...
class EmberDataModule(LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            data_name = get_dataloader(self.hparams.data_name)
            (X_train, X_val, X_test, y_train, y_val, y_test) = data_name.load(
            ...
            # data info
            log.warning(f"Train Data: {data_info(y_train)}")
            log.warning(f"Test Data: {data_info(y_test)}")
            log.warning(f"Val Data: {data_info(y_val)}")
